...
| cjk_fallback_without_system_font_installs_nothing | 无系统字体时不接入 | 给定缺失字体字节，当接入字体定义，则接入失败且字体表与字体族均保持内置原样 | 2026-09-23 |
| cjk_fallback_shares_bytes_across_contexts | 后备字体字节跨上下文共享 | 给定同一段字体字节接入两份字体定义，当检查，则两份定义都按借用登记且指向同一地址 | 2026-09-23 |
| system_cjk_font_is_discoverable | 系统 CJK 字体可发现 | 给定真实系统，当执行 CJK 字体发现，则必须找到（依赖宿主机） | 2026-09-23 |
| system_cjk_font_is_loaded_once | 系统 CJK 字体只加载一次 | 给定真实系统，当连续两次取字体字节，则两次返回同一段内存（依赖宿主机） | 2026-09-23 |
