...
以下 11 条一律不可协商——按优先级降序排列（定义见「代码评审原则与门禁对照」）只为在取舍冲突时指明先保哪条，级别低不等于可以放松。
...
9. **[LOW] 注释纪律**：代码内注释基于代码逻辑，准确描述代码的行为——这段代码在做什么、契约与单位、边界条件；「为什么」（决策、备选、历史、取舍）一律外置到本地 docs/comments/ 目录，SAFETY 注释除外。判据与命名、锚定规则见「注释纪律」一节。
...
约束「注释纪律」的细则。判据只有一条——**长期维护价值**：注释与外置记录都是写给未来维护者（包括未来的自己）的，只写能降低未来理解与修改成本的；一条注释该不该写、该留在代码还是外置，都以此判断。
...
评审（人与 AI）按下表执行：**能机械化的条目已全部落入门禁**——本地 `just precommit` 与 CI 跑同一套配方（ci.yml 与 hooks.yml 里每道门禁一个独立 job），规则一律由 workspace lints / 脚本强制，不靠口头约定。发现可机械化的新检查项时，优先补门禁而不是写进评审清单。
...
表里与约束同名的行就是该约束的门禁实现，其余行是评审层面的额外强制项。

### 人工评审关注点（无可靠机械门禁，勿硬造）

约束条目里的「纯 Rust 技术栈」「组装点唯一」与「注释纪律」也归这一节管：它们的判断没有可靠的机械门禁（纯 Rust 技术栈的判断发生在「要不要引入这个新依赖」的评审现场，枚举包名的 deny 名单覆盖不了没见过的运行时；注释纪律要判的是「有没有长期维护价值、该外置的「为什么」有没有外置」，不是措辞），同样靠人工评审，勿硬造检查。
...
选层先问一句：**这条断言需要什么条件才成立？** 不要为了少写一个测试替身把测试塞进更贵的层，也不要把需要真机的测试硬塞进 CI。源码注释里的 **L1–L4** 就是这套分层（`Cargo.toml`、`kittest.toml` 与各测试模块都在用），调层要一起改。

| 层 | 断言需要什么 | 形态与跑法 |
| --- | --- | --- |
| L1 | 纯逻辑或进程内时序 | 单元测试与库级集成，随 `just test` 进 CI |
| L2 | egui 渲染 | kittest UI harness，随 `just test` 进 CI |
| L3 | 窗口服务与 GPU | 显隐自检 `just selftest`，本地跑 |
| L4 | 真实 OS 授权或外部凭据 | live 测试 opt-in（`cargo test -- --ignored`），不进 CI |
...
基准不属于这四层：当前没有基准目标，也没有引入基准框架。要量化性能时新增 criterion 目标（放 benches 目录），**不要**把计时断言塞进 `#[test]`——CI 的负载波动会让它变成 flaky 门禁。基准数字要连同运行环境一起记录，CI 上只跑不判定趋势。
