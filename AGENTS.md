...
9. **[LOW] 注释纪律**：代码内注释只保留三类——功能介绍、SAFETY、一句话不变量/约束；决策过程与历史叙事外置到本地 `docs/comments/`。
   - **功能介绍进文档注释**：pub 项 `///`、模块级 `//!`，说清职责与边界；模块注释压缩到三五句，只讲职责，不讲来历（公共 API 的门禁见约束「公共 API 有文档注释」）。
   - **不变量/约束一句话留代码**：删掉就答不出「为什么这样写」的（单位、生命周期、防回归要点），压缩成一两句留下。
   - **决策过程与历史叙事外置**：背景、备选方案对比、任务/PR/评审引入的来龙去脉，写进 `docs/comments/<文件名>_comment.md`，按符号锚定（不写行号），改到相关代码时同步更新；mod.rs 同名冲突带父目录前缀。docs/ 整目录本地 gitignore，被跟踪文件（含代码）不留指向外置决策记录的指针，决策记录仅本地可见。
   - **注释不能与代码矛盾**：对外置 md 同样适用——过期的注释比没有注释更糟，它会把读者引向错误的方向。改代码时同步改注释，属于这次改动本身，不是收尾的礼貌。
   - **澄清，而不是添障碍**：留不下一句话「为什么」就别硬留在代码里；外置记录写不出增量信息也别写。SAFETY 注释必须紧贴 unsafe 块，永不外置。
...
约束条目里的「纯 Rust 技术栈」「组装点唯一」与「注释纪律」**不在上表**：它们的判断没有可靠的机械门禁（纯 Rust 技术栈只作口头约束——这类判断发生在「要不要引入这个新依赖」的评审现场，枚举包名的 deny 名单覆盖不了没见过的运行时；注释纪律要判的是「该外置的叙事有没有外置、注释含外置 md 有没有跟代码打架」，不是措辞），靠人工评审，别为它们硬造检查。表里与约束同名的行就是该约束的门禁实现，其余行是评审层面的额外强制项。
