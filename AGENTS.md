...
开发按里程碑推进，每个里程碑走一轮 PR 评审循环（分支一律自最新 main 切出，开工前先 fetch 确认远端可用）：

1. **新开分支**：沿用 `<type>/<序号>-<slug>` 命名（形如 feat/15-auto-classify，序号接续既有分支递增）。
2. **执行开发**：实现该里程碑全部条目与测试，本地过 `just check`；新增或修改的测试在同一 PR 内登记 `docs/tests/bdd.md`。
3. **提交 PR**：push 后经 `gh` 建 PR，描述写明变更范围、契约变化与测试清单。
4. **审查**：以 code-reviewer 评审该 PR（AI subagent 或人工皆可），意见按质量条目定级。
5. **修复循环**：按评审意见在同一分支补 commit 并重新请求审查，重复 2–4 直到评审无未决问题；合并由用户决定。
