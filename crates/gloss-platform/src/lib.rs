...
/// L4 opt-in 真机测试的授权前置检查（仅测试编译）。
///
/// rdev 监听/注入与 AX 读选区都要求**运行测试的进程**（通常是终端或
/// IDE）持有辅助功能授权；缺授权时系统表现为静默忽略，各测试会以不同
/// 的失败形态超时——这里统一前置拦截并给出可操作的授权步骤。
...
pub(crate) mod live_test_support {
    ...
    /// 前置断言：未授权时以可操作的修复指引快速失败，而不是让各测试
    /// 以「注入被忽略 / tap 未建立」的间接形态超时。
