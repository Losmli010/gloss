...
//! 侧解析任务选项；platform 侧的引擎读 keychain 条目标识与密钥，随 M4-T4
//! 注入），而 platform 只依赖 core（依赖方向红线）——句柄必须放在两者共同的
//! 可见层。组装点负责注入：M4-T3 接的是 app 侧，platform 侧随 M4-T4 接。
//!
//! **模型不经此解析**：M4-T4 的引擎从 `Task::options` 取模型（App 已在触发时
//! 按 `Config::model_by_kind` 解析并随任务下发），实现方不得回读配置——否则
//! 「缓存 key 用的模型」与「实际请求的模型」可能来自两份快照（见
//! [`crate::ports::AiEngine`]）。
//!
//! 三条不变量：
